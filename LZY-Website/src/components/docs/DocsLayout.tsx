...
/** The documentation sidebar as a drawer, on screens too narrow for a column. */
function DocsDrawer({ onClose, pathname }: { onClose: () => void; pathname: string }) {
    ...
    <div className="fixed inset-0 z-[80] lg:hidden" id="docs-drawer">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6">
          <DocSidebar key={pathname} onNavigate={onClose} />
        </div>
      </div>
    </div>
  );
...
export function DocsLayout({
  ...
  return (
    <div className="mx-auto max-w-[90rem] px-4 sm:px-6 lg:px-8">
      ...
      {drawer && <DocsDrawer onClose={() => setDrawer(false)} pathname={pathname} />}
